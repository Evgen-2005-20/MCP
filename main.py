...
def create_task(title: str, desc: str, deadline: str):
    return task_controller.create_task(db=db, title=title, desc=desc, deadline=deadline)

@mcp.tool()
def delete_task(id_task: int):
    return task_controller.delete_task(db=db, id_task=id_task)

@mcp.tool()
def update_task(id_task: int, title=None, desc=None, deadline=None, done=None):
    return task_controller.update_task(
        db=db, id_task=id_task, title=title, desc=desc, deadline=deadline, done=done
    )
...
def get_one_task(id_task: int):
    return task_controller.get_one_task(db=db, id_task=id_task)
